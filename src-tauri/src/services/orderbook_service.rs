    ...
    fn get_sandbox_orderbook(state: &AppState) -> Result<OrderbookResult> {
        ...
        let orders: Vec<Order> = sandbox_orders
            ...
            .map(|so| Order {
                ...
                side: so.side,
                quantity: so.quantity as f64,
                filled_quantity: so.filled_quantity.unwrap_or(0) as f64,
                pending_quantity: (so.quantity - so.filled_quantity.unwrap_or(0)) as f64,
                price: so.price,
                trigger_price: 0.0,
    ...
    fn get_sandbox_tradebook(state: &AppState) -> Result<TradebookResult> {
        ...
        let trades: Vec<Order> = sandbox_orders
            ...
            .map(|so| Order {
                ...
                exchange: so.exchange,
                side: so.side,
                quantity: so.filled_quantity.unwrap_or(so.quantity) as f64,
                filled_quantity: so.filled_quantity.unwrap_or(so.quantity) as f64,
